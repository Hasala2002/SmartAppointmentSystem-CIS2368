...
DO $$ BEGIN
  CREATE TYPE last_dental_visit_enum AS ENUM ('within-6-months','6-12-months','over-a-year','never');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
-- Backfill columns for pre-existing databases that were created before these
-- columns were added. Safe to run on a fresh DB too.
ALTER TABLE users ADD COLUMN IF NOT EXISTS date_of_birth date;
ALTER TABLE users ADD COLUMN IF NOT EXISTS has_dental_insurance boolean;
...
CREATE TABLE IF NOT EXISTS appointments (
  ...
  notes text,
  last_dental_visit last_dental_visit_enum,
...
-- Backfill columns for pre-existing databases that were created before these
-- appointment-intake fields were added. Safe on a fresh DB.
ALTER TABLE appointments ADD COLUMN IF NOT EXISTS last_dental_visit last_dental_visit_enum;
ALTER TABLE appointments ADD COLUMN IF NOT EXISTS has_dental_pain boolean;
